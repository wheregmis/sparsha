...
impl Widget for Checkbox {
    ...
    fn paint(&self, ctx: &mut PaintContext) {
        ...
        if self.checked {
            ...
            let mark_bounds = spark_core::Rect::new(
                bounds.x + mark_inset,
                bounds.y + mark_inset,
...
mod tests {
    ...
    use spark_input::{FocusManager, Key, KeyboardEvent, NamedKey};
    use spark_layout::LayoutTree;
    use std::sync::atomic::{AtomicBool, Ordering};
    use std::sync::Arc;

    fn checkbox_env() -> (LayoutTree, FocusManager, spark_layout::ComputedLayout) {
    ...
    fn keyboard_activate_toggles_when_focused() {
        ...
        let event = spark_input::InputEvent::KeyDown {
