...
pub fn init_web() -> Result<(), AppRunError> {
    ...
    if let Err(err) = console_log::init_with_level(log::Level::Debug) {
        web_sys::console::warn_1(
            &format!("sparsha logger already initialized or unavailable: {err}").into(),
        );
    }
    log::info!("Sparsha web platform initialized");
    Ok(())
}
    ...
    ($main_fn:ident) => {
        ...
        pub fn wasm_start() -> Result<(), wasm_bindgen::JsValue> {
            $crate::init_web().map_err(|err| {
                let message = format!("failed to initialize Sparsha web runtime: {err}");
                web_sys::console::error_1(&message.clone().into());
                wasm_bindgen::JsValue::from_str(&message)
            })?;

            $main_fn().map_err(|err| {
                let message = format!("failed to start Sparsha app: {err}");
                web_sys::console::error_1(&message.clone().into());
                wasm_bindgen::JsValue::from_str(&message)
            })
        }
