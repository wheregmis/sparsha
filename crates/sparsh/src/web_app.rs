//! Web runtime that renders Sparsh draw commands into the DOM.
...
impl WebTextInputBridge {
    fn new(
        ...
        element.set_class_name("sparsh-text-editor-bridge");
    ...
    fn sync(&mut self, editor_state: Option<&sparsh_widgets::TextEditorState>) {
        self.syncing = true;
...
fn paint_widget_subtree(
    ...
    if let Some(layout) = layout_tree.get_absolute_layout(id) {
        if let Some(surface) = widget.draw_surface() {
            ...
            let mut surface_ctx = sparsh_widgets::DrawSurfaceContext {
                draw_list: &mut surface_draw_list,
                bounds: sparsh_core::Rect::new(
                    0.0,
                    0.0,
...
impl WebAppState {
    ...
    fn emit_first_paint_event(&mut self) {
        ...
        match CustomEvent::new("SparshFirstPaint") {
            Ok(event) => {
                let _ = window.dispatch_event(event.as_ref());
            }
            Err(err) => {
                log::warn!("failed to emit SparshFirstPaint event: {:?}", err);
            }
        }
    }
...
fn install_event_listeners(
...

    {
        ...
        let on_key_down = Closure::wrap(Box::new(move |event: WebKeyboardEvent| {
            if let Some(key) = map_browser_key(event.key()) {
                let code = sparsh_input::ui_events::keyboard::Code::Unidentified;
                let mut kb_event = sparsh_input::KeyboardEvent::key_down(key, code);
...

    {
        ...
        let on_key_up = Closure::wrap(Box::new(move |event: WebKeyboardEvent| {
            if let Some(key) = map_browser_key(event.key()) {
                let code = sparsh_input::ui_events::keyboard::Code::Unidentified;
                let mut kb_event = sparsh_input::KeyboardEvent::key_up(key, code);
                kb_event.modifiers = browser_modifiers(&event);
                state
                    .borrow_mut()
...

    {
        ...
        let on_cut = Closure::wrap(Box::new(move |event: ClipboardEvent| {
            ...
            if clipboard_text.is_none() && fallback_text.is_some() {
                let mut synthetic = sparsh_input::KeyboardEvent::key_down(
...
mod tests {
    ...
    use sparsh_layout::taffy;
    use sparsh_widgets::DrawSurface;

    struct HybridOverlayWidget {
        id: sparsh_layout::WidgetId,
        surface: DrawSurface,
    }
    ...
    impl HybridOverlayWidget {
        fn new() -> Self {
            Self {
                id: sparsh_layout::WidgetId::default(),
    ...
    impl Widget for HybridOverlayWidget {
        fn id(&self) -> sparsh_layout::WidgetId {
            self.id
        }

        fn set_id(&mut self, id: sparsh_layout::WidgetId) {
            self.id = id;
        }
